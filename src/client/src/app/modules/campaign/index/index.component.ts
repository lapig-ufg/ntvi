...
export class IndexComponent implements AfterViewInit {
    ...
    colorScheme = {
        domain: ['#858585', '#5AA454', '#C7B42C', '#A10A28'],
    };
    ...
    canInspect(campaign) {
        ...
        const userOnCampaign = campaign.UsersOnCampaigns.find(userInCampaign => userInCampaign.user.id === currentUser.id);
        return (
    ...
    canOpenResults(campaign) {
        ...
        campaign.UsersOnCampaigns.forEach(function (item) {
            if (item.user.id.toString() === currentUser.id.toString() && item.typeUserInCampaign === 'ADMIN') {
                can = true;
            }
        });
        return can;
    ...
    getPointsCacheData(campaign) {
        const processingPoints = campaign.pointsCacheStatus.processingPoints;
        const cachedPoints = campaign.pointsCacheStatus.cachedPoints;
        const failedPoints = campaign.pointsCacheStatus.failedPoints;
        const pendingPoints = campaign.pointsCacheStatus.pendingPoints;

        return [
            {
                'name': 'Processing',
                'value': processingPoints,
            },
            {
                'name': 'Cached',
                'value': cachedPoints,
