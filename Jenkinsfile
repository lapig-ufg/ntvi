 node {
    ...
    def NODE_VERSION="v14.20.0"
        ...
        stage('Checkout') {
            git branch: 'main',
            url: 'https://github.com/lapig-ufg/ntvi.git'
        }
        stage('Validate') {
        ...
        stage('Build') {
                        ...
                        def status = sh(returnStatus: true, script: "cd src/client && ng build -c production --stats-json --source-map=false --no-progress")
                        if (status != 0) {
                            echo "FAILED BUILD!"
